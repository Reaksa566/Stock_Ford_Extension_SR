...
const API_URL_BASE = '/api/accessories';
...
const StockModal = ({ isOpen, onClose, initialData, isView, itemType, fetchItems }) => {
  ...
  useEffect(() => {
    setData(initialData || { description: '', unit: '', stockIn: 0, stockOut: 0 });
    setQuantityChange(0);
    setReference('');
    setError('');
  }, [initialData, isOpen]);
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl transform transition-all scale-100 duration-300">
        ...
        <div className="p-5 border-b flex justify-between items-center bg-ford-blue text-white rounded-t-xl">
          <h3 className="text-xl font-bold">
            {isView ? 'មើលព័ត៌មានលម្អិត' : isEdit ? `កែសម្រួលគ្រឿងបន្លាស់` : `បន្ថែមគ្រឿងបន្លាស់ថ្មី`}
          </h3>
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
        ...
        <div className="p-5 border-b flex justify-between items-center bg-ford-blue text-white rounded-t-xl">
          <h3 className="text-xl font-bold">
            <FaFileUpload className="inline mr-2" /> នាំចូលគ្រឿងបន្លាស់ពី Excel
          </h3>
          <button onClick={() => onClose(false)} className="text-white hover:text-gray-300">
            <FaTimes className="w-5 h-5" />
          </button>
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-5 space-y-4">
            <p className="text-sm text-gray-600">
              ឯកសារ Excel ត្រូវតែមានជួរឈរ (Columns) ដូចខាងក្រោម៖ **Description**, **Unit**, **Stock In**។ ជួរឈរ **Stock Out** អាចទុកចោលបាន។
...
const AccessoryPage = () => {
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className="text-3xl font-bold text-gray-800">គ្រប់គ្រងគ្រឿងបន្លាស់ (Accessory Management)</h1>
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        {!loading && filteredData.length === 0 && (
          <div className="text-center p-8 text-gray-500">មិនមានទិន្នន័យគ្រឿងបន្លាស់ទេ។</div>
        )}
      </div>
      ...
      <StockModal
        ...
        isView={isViewMode}
        itemType="គ្រឿងបន្លាស់"
        fetchItems={fetchItems} // Pass fetchItems to modal
      />
